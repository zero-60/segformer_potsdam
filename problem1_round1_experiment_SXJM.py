...
def args():
    ...
    p.add_argument("--data-dir", type=Path, default=Path(__file__).resolve().parent / "real_attachments" / "A_data_value")
    p.add_argument("--out-dir", type=Path, default=Path(__file__).resolve().parent / "round1_outputs")
...
def locate(root, name, pattern):
    """Locate a dataset file under A_data_value, including its subfolders."""
    p = root / name
    if p.exists():
        return p

    # Search recursively because the contest package stores some files under
    # slimpajama_quality_extended/ and regmix_tables/.
    ...
    if len(hits) == 0:
        raise FileNotFoundError(
            ...
            f"Recursive pattern used: {pattern}"
        )
    raise RuntimeError(
        f"More than one file matches {pattern} under {root}:\n"
...
def main():
    ...
    print("1/8 正在寻找并读取解压数据文件 (耗时最长，请耐心等待)...")
    paths = {
    ...
    print("2/8 数据加载完成，正在生成基础统计与列表长度信息...")
    ...
    print("5/8 正在进行缩尾截断 (Winsorize) 与归一化处理...")
    ...
    print("6/8 正在计算 Spearman 相关性并执行 PCA 主成分分析...")
    corr = z["A1"][QUALITY_COLS].corr(method="spearman")
    corr.to_csv(tab / "08_A1_spearman.csv");
    corrplot(corr, fig / "01_A1_correlation.png")
    ...
    f, a1 = plt.subplots(figsize=(8, 5));
    xx = np.arange(1, 23)
    a1.plot(xx, ev, marker="o", label="Individual");
    a1.plot(xx, np.cumsum(ev), marker=".", label="Cumulative")
    a1.axhline(.8, ls="--");
    a1.axhline(.9, ls="--");
    a1.set_xlabel("Principal component")
    a1.set_ylabel("Explained variance ratio");
    a1.set_title("PCA diagnostic");
    a1.legend()
    f.tight_layout();
    f.savefig(fig / "02_PCA_scree.png", dpi=220);
    plt.close(f)

    print("7/8 正在进行 KS 检验与分布偏移计算...")
    ...
    n80 = int(np.argmax(np.cumsum(ev) >= .8) + 1);
    n90 = int(np.argmax(np.cumsum(ev) >= .9) + 1)
    summary = [
        ...
        sa.nlargest(5, "ks_statistic")[
            ["feature", "ks_statistic", "standardized_mean_difference", "ks_fdr_bh"]].to_string(index=False), "",
        "## Largest github shifts",
        sg.nlargest(5, "ks_statistic")[
            ["feature", "ks_statistic", "standardized_mean_difference", "ks_fdr_bh"]].to_string(index=False), "",
        "## Interpretation rule",

...
# -*- coding: utf-8 -*-
"""
Formal training for Model C / Quality Gate / Ours.

Architecture
------------
Same dual SegFormer encoders + shared decoder + four fusion locations as
Model C-noGate.

Only architectural change:
    fixed 0.5/0.5 fusion
        ->
    dynamic sample-wise per-scale Quality Gate fusion.

Training protocol
-----------------
Kept aligned with A / B / C-noGate:
...
- same external six-class cross entropy.
- same mmap data cache.

No degradation augmentation.
No gate auxiliary loss.
No gate regularization.

Quality Gate training diagnostics
---------------------------------
Each epoch logs per scale:

    w_RGB mean / std / min / max
    w_NIR mean
    gate parameter L2 norm

This is training diagnostic data only. Formal RQ5 interpretation should be
performed on the frozen validation set under Clean / L1 / L2 / L3, because
that directly tests whether w_RGB decreases as RGB quality deteriorates.

Expected location:
    tools/train_model_c_quality_gate.py
...
Run:
    python models/segformer_dual_quality_gate.py
    python tools/train_model_c_quality_gate.py

Outputs:
    outputs/training/model_c_quality_gate/
        protocol.json
        train_log.jsonl
...
try:
    ...
    raise ImportError(
        ...
        "Keep that file in tools/ before training Model C."
    ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train Model C / Ours: "
            "dual encoder + dynamic multi-scale Quality Gate."
        ),
        formatter_class=(
            argparse
...
def build_protocol(
    *,
    ...
    return {
        ...
        "variant": "C",
        "scientific_role": (
            "proposed dual-encoder dynamic multi-scale Quality Gate"
        ),
        "input_modalities": [
            "RGB",
            "NIR",
        ...
        "gate_auxiliary_supervision": False,
        "gate_regularization": None,
        "clean_training_only": True,
        "degradation_augmentation": False,
        "num_classes": (
            NUM_CLASSES
        ),
        "ignore_index": (
        ...
        "controlled_against_c_nogate": {
            "same_common_pretrained_initialization": True,
            "trained_c_nogate_checkpoint_loaded": False,
            "same_rgb_encoder": True,
            "same_nir_encoder": True,
            "same_nir_first_layer_initialization": True,
            "same_shared_decoder": True,
            "same_four_fusion_locations": True,
            ...
            "only_intended_architecture_change": (
                "fixed 0.5/0.5 modality weights become dynamic "
                "sample-wise per-scale Quality Gate weights"
            ),
        },
    }
    ...
    *,
    ...
    if isinstance(
        ...
        if protocol.get(
            ...
            raise RuntimeError(
                "Resume protocol is not Model C / Quality Gate."
            )
        ...
        if not bool(
            ...
            raise RuntimeError(
                "Resume checkpoint does not enable Quality Gate."
            )
...
def validate_first_batch(
    *,
    ...
    if (
        ...
        raise RuntimeError(
            "Expected RGB=float32, NIR=float32, labels=int64; "
...
def validate_model_output(
    *,
    ...
    if tuple(
        ...
        raise RuntimeError(
            "Quality Gate weight tensor invalid: "
    ...
    if tuple(
        ...
        raise RuntimeError(
            "Quality Gate logit tensor invalid: "
    ...
    if pair_sum_error > 1e-6:
        raise RuntimeError(
            "Quality Gate weights do not sum to one: "
            f"max_error={pair_sum_error}."
        )

    if expect_exact_half:
        if not torch.equal(
            weights,
            torch.full_like(
                weights,
            ...
            raise RuntimeError(
                "Fresh Model C does not start at exact fixed "
                "0.5/0.5 fusion."
            )
        ...
        if not torch.equal(
            ...
            raise RuntimeError(
                "Fresh Model C gate logits are not exactly zero."
            )

    print(
        "[check] model output OK | "
        f"logits={tuple(logits.shape)} {logits.dtype} | "
        f"w_RGB(mean by scale)="
        f"{weights[:, :, 0].detach().float().mean(dim=0).cpu().tolist()}"
    )


class GateEpochStats:
    """
    Streaming per-scale gate statistics over training samples.
    """
    ...
    def update(
        ...
        if (
            fusion_weights.ndim
            != 3
            or fusion_weights.shape[1]
            != self.num_scales
            or fusion_weights.shape[2]
            != 2
        ):
            raise RuntimeError(
                "GateEpochStats received invalid fusion weight shape: "
                f"{tuple(fusion_weights.shape)}."
            )
    ...
    def finalize(
        ...
        if self.count <= 0:
            raise RuntimeError(
                "No Quality Gate samples were accumulated for this epoch."
            )
...
def gate_parameter_l2(
    ...
    if not parameters:
        raise RuntimeError(
            "No Quality Gate parameters found in Model C."
        )
...
def main() -> None:
    ...
    print(
        "[3] building Model C / Quality Gate"
    )
    ...
    print(
        "  gate type              : "
        f"{model_meta['gate']['type']}"
    )
    print(
        "  gate parameters        : "
        f"{model_meta['gate_parameters']:,}"
    )
    print(
        "  total parameters       : "
    ...
    print(
        "  initial w_RGB / w_NIR  : "
        f"{model_meta['gate']['initial_w_rgb']:.1f} / "
        f"{model_meta['gate']['initial_w_nir']:.1f}"
    )
    print(
    ...
    print(
        "[protocol] clean training only | "
        "degradation augmentation=False | "
        "gate auxiliary loss=False"
    )
    ...
    for epoch in range(
        ...
        for batch_index, batch in enumerate(
            ...
            with torch.autocast(
                ...
                # Reuse this already-computed batch for logging. Do NOT run
                # an extra forward just to inspect the gate: an extra training
                # forward would consume dropout RNG and weaken the controlled
                # comparison with C-noGate.
                current_gate_for_log = (
                    fusion_weights[
                        :,
                        :,
                        0,
                ...
                # Keep only the main logits graph. The fused logits already
                # contain the necessary gradient path through gate weights.
                del fusion_weights
                del details
            ...
            if (
                ...
                print(
                    ...
                    f"wRGB "
                    f"{[round(v, 4) for v in current_gate_for_log]}"
                )
        ...
        print(
            ...
            f"{current_lr:.3e} | "
            f"wRGB_mean="
            f"{[round(v, 4) for v in gate_summary['w_rgb_mean_by_scale']]} | "
            f"gate_l2="
            f"{gate_l2:.6f} | "
            f"time="
